...
> 当前分工：本对话继续负责前端体验与确定性规则；后续新对话负责 MiniMax 模型适配与接入。

## 1. 当前产品闭环

目前已经跑通以下完整流程：

> 输入自然语言 → 解析结构化行程 → 确认歧义地点 → 调用高德路线与天气 → 计算时间、能耗和主动服务 → 15 秒加速演示
...
- 解析结果可以在“我理解的行程”中修改日期、时间类型、目标时间、起点和途经点，再重新规划。
...
- 需要温控备车时额外计入 1.5 kWh。
- Demo Lab 开启时，使用设置的座舱温度判断预热或制冷；关闭时使用高德室外温度。
- 座舱温度 `≤10°C`：提前 15 分钟预热，并建议座椅加热。
- 座舱温度 `≥30°C`：提前 12 分钟制冷。
- 雨、雪、雷天气：增加除雾和雨具提醒。
- 预计到达电量 `<20%`：补能提醒；`<10%`：高风险提醒。
- “允许 NOMI 根据天气主动备车”关闭后，不生成预热、制冷和座椅加热动作。
- 所有路线、ETA、天气、能耗和动作均由程序计算，不由模型生成。

### 2.4 Demo Lab

所有默认设置集中在右上角 Demo Lab：

- 开关：允许 NOMI 根据天气主动备车。
- 收藏地点：家、我的公司、儿子学校、老婆公司。
- 每个收藏地点都可重新搜索高德 POI 并修改具体地址。
- 演示场景开关及覆盖项：天气状况、座舱温度、当前电量。
- 室外温度没有单独的手动输入项；实时模式下仍由高德天气提供。
- 设置保存在浏览器 `localStorage`，刷新页面后继续生效。
- 演示场景开启时，右上角立即显示设置的天气与座舱温度，例如“小雪 8°”；关闭后显示高德天气与室外温度。

### 2.5 15 秒加速演示

- 演示时长固定为 15 秒，不提供 15/30/60 秒选择。
- 状态机：`DRAFT → PLANNED → CONFIRMED → PRECONDITIONING → READY → EN_ROUTE → AT_STOP → EN_ROUTE → COMPLETED`。
- 车辆沿高德 Polyline 移动，各段行驶时间按真实路线耗时比例分配。
- 地图、当前目的地、NOMI 提示、阶段状态和剩余电量同步变化。
- 右上角在规划阶段不显示系统实时时钟；开始演示后显示“模拟”时间，并随演示进度推进。
- 可从头重新播放 15 秒演示。

### 2.6 页面与适配

- Next.js + React + TypeScript 单体全栈项目。
- 横屏类车机布局：左侧地图、右侧 NOMI 与行程卡、底部自然语言输入。
- NOMI 灵感视觉，未使用或复刻官方品牌资产。
- 主要适配 Chrome 1920×1080 与 1440×900 横屏。
- 行程、计划和 Demo Lab 设置通过 `localStorage` 保存，当前不使用数据库或 WebSocket。
...
| `src/components/Cockpit.tsx` | 主页面、编辑器、Demo Lab 和演示状态 |
| `src/components/AmapMap.tsx` | 高德地图、路线和车辆动画 |
...
- 车辆状态全部为模拟数据。
- 当前只支持单轮完整行程句式；复杂追问、多轮补充和长期记忆尚未实现。
